...
function cekNominalnya(){
  ...
  if ( kategori_barang == '' ) {
     ...
     alert('Pilih kategori barang terlebih dahulu');
      ...
      $.post("<?php echo base_url('Transaksi/list_master_barang');?>",{tkgid:kategori_barang,is_stok:'t'},function(data){

            var hasil = JSON.parse(data);
            var num = 0;
            var jml_data = hasil.length;
            $.each(hasil,function(k,v){
                addrowmodal(v.tbrid,v.nama_barang,v.nama_satuan,v.hna,v.satuan,v.stok);
                num = num + 1;
            });
...
function addrowmodal(tbrid,nama_barang,nama_satuan,hna,tsid,stok) {
    ...
    var newCell = row.insertCell(4);
        newCell.align ="right";
        newCell.innerHTML = stok+"<input type='hidden' name='stok[]' id='stok["+tbrid+"]' value='"+stok+"'/>";        
...
function addrow(tbrid,nama_barang,satuan,hna,nama_satuan){
  ...
  row.setAttribute('class','barang-'+lastRow);
  ...
  var newCell = row.insertCell(1);
      newCell.align ="center";
      newCell.innerHTML = "<input type='hidden' name='tbrid[]' id='tbrid["+lastRow+"]' class='form-control' value='"+tbrid+"'/><input type='hidden' name='nama_barang[]' id='nama_barang["+lastRow+"]' class='form-control' value='"+nama_barang+"'/>"+nama_barang;


  var newCell = row.insertCell(2);
      newCell.align ="center";
      newCell.innerHTML = "<input type='text' name='jumlah[]' id='jumlah["+lastRow+"]' class='form-control' style='text-align:center' onblur='hitungSubtotal("+lastRow+");'/>";
...
function hitungSubtotal(id) {
    ...
    var tbrid = document.getElementById('tbrid['+id+']').value;
    var nama_bar = document.getElementById('nama_barang['+id+']').value;
    var qty     = document.getElementById('jumlah['+id+']').value;

    $.post("<?php echo base_url('Transaksi/cek_stok')?>",{tbrid:tbrid},function(data){

          if ( data == 'null' ) {

              alert('Stok Kosong Barang '+nama_bar+' Tidak ada, Barang Dihapus Dari List');
              $("table#table_barang tbody tr.barang-"+id).remove();

          }else{

              var hasil = JSON.parse(data);

              if ( parseFloat(hasil.stok) < parseFloat(qty) ) {

                    alert('Stok Barang '+nama_bar+' Tidak Mencukupi');
        ...
        <div class="row">
          <div class="col-12">
            <div class='card'>
              ...
              <form class='form-horizontal' name='frm' id='frm' method='post' action="<?php echo base_url('transaksi/act_add_pemakaian'); ?>">
                  <div class='card-body'>
                    ...
                    <div class='form-group-row'>
                      ...
                      <div class='col-3' style="float:left">
                          <label class="col col-form-label">Tanggal Pemakaian</label>
                          <div class="col">
                            <div class="input-group">
                             ...
                             <input type="text" class="form-control float-right" name="tgl_trans" id="reservation">
                            </div>
                          </div>
                      </div>

                     <div class='col-3' style="float:left" hidden>

                           <label for="inputEmail3" class="col col-form-label">Kategori Barang</label>
                                
                            <select name='tkgid' id='tkgid' class='form-control' onchange="clearListOrder();" required="">
                              <option></option>
                              <?php 
