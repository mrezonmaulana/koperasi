...
function addrow(tbrid,nama_barang,satuan,hna,nama_satuan,jumlah=0,handfeel="",no_match=""){
  ...
  var chk_super = chk_soft = chk_isi = "";

  if ( handfeel == 'SUPER SOFT' ) {
      chk_super = "selected='selected'";
  } else if ( handfeel == 'SOFT' ) {
      chk_soft = "selected='selected'";
  } else if ( handfeel == 'SOFT ISI') {
      chk_isi = "selected='selected'";
  }

  var combo_handfeel = "<select name='handfeel[]' id='handfeel[]' class='form-control form-control-sm'><option></option>";
        combo_handfeel += "<option value='SUPER SOFT' "+chk_super+"> SUPER SOFT</option>";
        combo_handfeel += "<option value='SOFT' "+chk_soft+"> SOFT</option>";
        combo_handfeel += "<option value='SOFT ISI' "+chk_isi+"> SOFT ISI</option>";
  ...
  var newCell = row.insertCell(1);
    ...
    var newCell = row.insertCell(3);
      newCell.align ="center";
      newCell.innerHTML = "<input type='text' name='no_match[]' id='no_match["+lastRow+"]' value='"+no_match+"' class='form-control form-control-sm' style='text-align:center' />";


  var newCell = row.insertCell(4);
      newCell.align ="center";
      newCell.innerHTML = "<input type='text' name='jumlah[]' id='jumlah["+lastRow+"]' value='"+jumlah+"' class='form-control form-control-sm' style='text-align:center' onblur='hitungSubtotal("+lastRow+");'/>";
          ...
          <div class="col-12">
            <div class='card'>
              ...
              <form class='form-horizontal' name='frm' id='frm' method='post' action="<?php echo base_url('transaksi/act_add_penjualan'); ?>">
                <input type='hidden' name='ttbid' id='ttbid' value='<?php echo $header->ttbid;?>'/>
                  <div class='card-body'>
                    ...
                    <div class='form-group-row'>
                      ...
                      <div class='col-3' style="float:left">
                          ...
                          <div class="col">
                            <div class="input-group">
                             ...
                             <input type="text" class="form-control form-control-sm float-right" name="tgl_trans" id="reservation"  readonly="readonly" value="<?php echo date('m/d/Y',strtotime($header->tgl_trans))?>">
                            </div>
                          </div>
                      </div>
                     ...
                     <div class='col-3' style="float:left" >
                           ...
                           <label for="inputEmail3" class="col col-form-label">No. SJ</label>
                             <input type="text" name="no_sj" id="no_sj" class="form-control form-control-sm" value="<?php echo $header->no_sj;?>"/>
                            <select name='tkgid' id='tkgid' class='form-control form-control-sm' onchange="clearListOrder();" required="" style="pointer-events: none" hidden>
                              <option></option>
                              <?php 
                      ...
                      <div class='col' style="float:left">
                            <label for="inputEmail3" class="col col-form-label">Keterangan</label>
                                <input type="text" class="form-control form-control-sm" name='keterangan' id='keterangan' value='<?php echo $header->keterangan; ?>' required="">
                      </div>
...
<?php foreach($detail->result_array() as $k){ ?>
    ...
    addrow("<?php echo $k['tbrid'];?>","<?php echo $k['nama_barang_new'];?>","<?php echo $k['tsid'];?>","<?php echo $k['harga_satuan'];?>","<?php echo $k['satuan'];?>","<?php echo $k['vol'];?>","<?php echo $k['handfeel'];?>","<?php echo $k['no_match']?>");
