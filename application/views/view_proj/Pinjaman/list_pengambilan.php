...
<script type="text/javascript">
  function tambahAnggota(tipe,id=""){
    $("#formAddPinjaman .modal-title").html('Tambah Pengambilan Simpanan');
    if ( tipe == 1 ) {
      $("#data-title-form").html('Tambah Pengambilan Simpanan');
      $("select[name='teid']").val('');
      $("td#contentKalkulasi").html('');
      $("a#btnKalkulasi").css('display','');
      $("input[name='tpsid']").val('0');
      ...
      $.post("<?php echo base_url('Pinjaman/detailPengambilan')?>",{teid:id},function(data){

          var newRes = JSON.parse(data);
          $("#data-title-form").html('Detail Pengambilan Simpanan');
          ...
          $("#sisaSaldo").html("Rp. "+formatRupiahNew(newRes[0].nominal_awal,'Rp. '));
          $("button#realAddAnggota").click();
  ...
  swalWithBootstrapButtons.fire({
    ...
    if (result.isConfirmed) {
      $.post("<?php echo base_url('Pinjaman/hapusPengambilan')?>",{tpsid:id},function(data){
        ...
        if ( resData.status == '200' ) {
          var timerInterval;
            Swal.fire({
              title: "Success!",
              html: "Data Pengambilan Simpanan Sukarela Berhasil Dihapus",
              icon:"success",
              timer: 2000,
  ...
  function convertXLS(tipe){
    if ( tipe == 1 ) {
      ...
      var url = "<?php echo base_url('Pinjaman/list_pengambilan')?>/";
        ...
        <div class="row">
          <div class="col-12">
            <div class='card'>
              ...
              <form method="post" name="frm_cari" id="frm_cari" action="<?php echo base_url('Pinjaman/list_pengambilan')?>">
                ...
                <div class="card-footer">
                  ...
                  <div style="float:right">
                    <button type="button" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#formAddPinjaman" id="realAddAnggota" style="display:none"><i class='fas fa-plus'></i> Tambah Pengambilan</button>
                    <button type="button" class="btn btn-danger btn-sm" id="dummyAddAnggota" onclick="tambahAnggota(1);"><i class='fas fa-plus'></i> Tambah Pengambilan</button>
                  </div>
                </div>
              </form>
            </div>
                    ...
                    <?php 
                          $no=1;
                           foreach($list->result_array() as $i) {
                              ...
                              echo "<td align='center'><a href='#' class='btn btn-warning btn-sm' title='Detail Pinjaman' onclick='tambahAnggota(2,".$i['tpsid'].");return false;'><i class='fas fa-eye'></i></a>&nbsp;<a href='#' class='btn btn-danger btn-sm' title='Hapus Anggota' onclick='hapusAnggota(".$i['tpsid'].");return false;'><i class='fas fa-trash'></i></a></td>";
                              echo "</tr>";
                              $no++;
                           } 
                        ?>
...
function saveData(){
   ...
   if ( data_teid == '' || data_teid == null || data_teid == 'null' ) {

      Swal.fire({
              title: "Oops!",
              text: "Anggota Belum Dipilih",
              icon: "warning"
            });
       return false;
   }

    if ( data_nominal_pinjam == 0 || data_nominal_pinjam == '' || data_nominal_pinjam == 'null' || data_nominal_pinjam == null ) {

      Swal.fire({
              title: "Oops!",
              text: "Jumlah Pengambilan Simpanan Belum Di isi",
              icon: "warning"
            });
       return false;
   }

   if ( parseFloat(data_nominal_pinjam) > parseFloat(sisa_saldo_nom) ) {
       Swal.fire({
              title: "Oops!",
              text: "Sisa saldo simpanan tidak mencukupi",
              icon: "error"
            });
       return false;
   }
...
function showSaldo(){
   ...
   $("input[name='sisa_saldo_nom']").val(saldo);
}
  ...
  <div class="modal-dialog modal-xl">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title" id='data-title-form'>Tambah Pengambilan Simpanan</h4>
      ...
      <div class="modal-body">
        <table width="100%" class="table no-border table-sm">
          <input type='hidden' name='tpsid' id='tpsid' value='0'>
              ...
              <td colspan="2">
                ...
                <?php 
                  foreach( $list_karyawan->result_array() as $i ){
                    echo "<option value='".$i['teid']."' data-saldo='".$i['saldo']."'>".$i['nik']." - ".$i['nama_karyawan']."</option>";
                  }
                ?>
              </select>
              </td>
