...
<script type="text/javascript">
  ...
  function tambahAnggota(tipe,id=""){
    $("#formAddPinjaman .modal-title").html('Tambah Pinjaman');
    if ( tipe == 1 ) {
      $("#data-title-form").html('Tambah Pinjaman');
      ...
      $("button[id='btn_save_anggota']").css('display','none');
  ...
  swalWithBootstrapButtons.fire({
    ...
    if (result.isConfirmed) {
      $.post("<?php echo base_url('Pinjaman/hapusKoperasi')?>",{tpid:id},function(data){
  ...
  function convertXLS(tipe){
    if ( tipe == 1 ) {
      ...
      var url = "<?php echo base_url('Pinjaman/list_koperasi')?>/";
        ...
        <div class="row">
          <div class="col-12">
            <div class='card'>
              ...
              <form method="post" name="frm_cari" id="frm_cari" action="<?php echo base_url('Pinjaman/list_koperasi')?>">
                ...
                <div class="card-footer">
                  ...
                  <div style="float:right">
                    <button type="button" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#formAddPinjaman" id="realAddAnggota" style="display:none"><i class='fas fa-plus'></i> Tambah Pinjaman</button>
                    <button type="button" class="btn btn-danger btn-sm" id="dummyAddAnggota" onclick="tambahAnggota(1);"><i class='fas fa-plus'></i> Tambah Pinjaman</button>
                  </div>
                </div>
              </form>
            </div>
                    ...
                    <?php 
                          $no=1;
                           foreach($list->result_array() as $i) {
                              ...
                              echo "<td align='center'><a href='#' class='btn btn-warning btn-sm' title='Detail Pinjaman' onclick='tambahAnggota(2,".$i['tpid'].");return false;'><i class='fas fa-eye'></i></a>&nbsp;<a href='#' class='btn btn-danger btn-sm' title='Hapus Anggota' onclick='hapusAnggota(".$i['tpid'].");return false;'><i class='fas fa-trash'></i></a></td>";
                              echo "</tr>";
                              $no++;
                           } 
                        ?>
...
function saveData(){
   var data_tpid = $("input[name='tpid_pinjaman']").val();
   var data_teid = $("select[name='teid']").val();
   var data_tanggal_pinjam = $("input[name='tgl_mulai']").val();
   var data_tenor = $("input[name='tenor']").val();
   var data_nominal_pinjam = backToNormal(document.getElementById('nominal'));
   var data_provisi = $("input[name='biaya_provisi']").val();
   var data_real_pinjam = $("input[name='jumlah_real_pinjaman']").val();
   var data_simpanan_wajib = $("input[name='nominal_simpanan_wajib']").val();
   var data_jasa_pinjam = $("input[name='nominal_jasa_pinjaman']").val();
   var data_total_piutang = $("input[name='total_piutang']").val();
   var data_jumlah_cicilan = $("input[name='jumlah_cicilan']").val();
   
   Swal.fire({
    title: "Apakah anda yakin?",
    text: "Pastikan Data Pinjaman Sudah Sesuai !",
    icon: "warning",
    showCancelButton: true,
    ...
    if (result.isConfirmed) {
      var dataSend = {
        ...
        provisi: data_provisi,
        real_pinjam: data_real_pinjam,
        simpanan_wajib: data_simpanan_wajib,
        jasa_pinjam: data_jasa_pinjam,
        total_piutang: data_total_piutang,
        jumlah_cicilan: data_jumlah_cicilan,
        
      };
      $.post("<?php echo base_url('Pinjaman/saveKoperasi')?>",dataSend,function(data){
      ...
      <div class="modal-body">
        <table width="100%" class="table no-border table-sm">
          <input type='hidden' name='tpid_pinjaman' id='tpid_pinjaman' value='0'>
            ...
            <tr>
              <td width="15%">Tenor ( Minggu )</td>
              <td>
                <input type="text" name="tenor" id="tenor" class="form-control form-control-sm">
              </td>
