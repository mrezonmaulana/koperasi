    ...
    if ( tpids != '' && tpids != ' ') {
        ...
        if ( ada === 0 ) {
            ...
            $.post("<?php echo base_url('Ajuan/detail_project')?>",{tpids:tpids},function(data){
                ...
                if ( parseFloat(data_proj.biaya_total_barang) > 0 ){
                  addRow(data_proj.tpid,data_proj.nama_project,data_proj.anggaran,data_proj.biaya_total_barang);

                  total_all_anggar = parseFloat(total_all_anggar) + parseFloat(data_proj.anggaran);
                  total_all_bar = parseFloat(total_all_bar) + parseFloat(data_proj.biaya_total_barang);

                  document.getElementById('total_all_anggaran').value = total_all_anggar;
                  document.getElementById('total_nilai_kontraknya').value = formatRupiah("'"+total_all_anggar+"'","Rp. ");
                  document.getElementById('total_all_barang').value = total_all_bar;
                  document.getElementById('total_nilai_barangnya').value = formatRupiah("'"+total_all_bar+"'","Rp. ");


                  sisa = total_all_anggar - total_all_bar;
                  document.getElementById('sisa_biaya').value = formatRupiah("'"+sisa+"'","Rp. ");
                } else {
                   alert('Project Belum Membeli Barang');
                }
            });
...
var newCell = row.insertCell(0);
      newCell.innerHTML = "<input type='hidden' name='tpid[]' id='tpid[]' class='form-control' value='"+tpid+"'/><input type='hidden' name='sisa_set_brg[]' id='sisa_set_brg[]' value='"+sisa_setelah_barang+"'/>"+nama_project;
...
function hapusProject(tpid,id) {
    ...
    var bar = parseFloat(document.getElementById('biaya_barang['+id+']').value);
    var total_all_anggar = parseFloat(document.getElementById('total_all_anggaran').value);
    var total_all_bar = parseFloat(document.getElementById('total_all_barang').value);
    var sisa_anggar = sisanya = sisa_bar = pengajuan = sisa_bawah = 0;

    sisa_anggar = total_all_anggar - anggar;
    
    
    sisa_bar    = total_all_bar - bar;
    
    
    sisanya = sisa_anggar - sisa_bar;

    pengajuan = backToNormal(document.getElementById('total_ajuan_project'));

    sisa_bawah = sisanya - parseFloat(pengajuan);

    if ( sisa_bawah < 0 ) {

    alert('Project Gagal Dihapus ! Karena Jika Project ini Dihapus Nominal Pengajuan Kas Akan Lebih Besar Dari Nominal Sisa Sebelum Ajuan Kas Project ');

    } else {

    document.getElementById('total_all_anggaran').value = sisa_anggar;
    document.getElementById('total_nilai_kontraknya').value = formatRupiah("'"+sisa_anggar+"'","Rp. ");
    document.getElementById('total_all_barang').value = sisa_bar;
    document.getElementById('total_nilai_barangnya').value = formatRupiah("'"+sisa_bar+"'","Rp. ");       
    document.getElementById('sisa_biaya').value = formatRupiah("'"+sisanya+"'","Rp. ");
    document.getElementById('sisa_biaya_project').value = formatRupiah("'"+sisa_bawah+"'","Rp. ");
      ...
      alert('Sisa Setelah Ajuan Project Tidak Boleh 0 atau kurang dari 0');
        ...
        if(confirm('Apakah anda yakin akan memproses ajuan ini ?')) {
            $.post("<?php echo base_url('Ajuan/act_add_ajuan');?>",{teid_pengaju:teid_pengaju,tgl_ajuan:tgl_ajuan,sisa_sbm_ajuan:sisa_sbm_ajuan,total_ajuan_proj:total_ajuan_proj,sisa_biaya_proj:sisa_biaya_proj,arrtpid:JSON.stringify(arr_tpid),arrajuan:JSON.stringify(arr_ajuan)},function(data){
                ...
                window.location.replace("<?php echo base_url('Ajuan/list_newajuan')?>");
            });
        }
    }
}
                              ...
                              <div class='col' style="float:left">
                                  ...
                                  <label class="col col-form-label">Tanggal Ajuan</label>
                                    <div class="input-group">
                                     ...
                                     <input type="text" class="form-control float-right" name="tgl_ajuan" id="tgl_ajuan" value='<?php echo date('m-d-Y',strtotime($pengajunya->tanggal_ajuan));?>'>
                                    </div>
                                  </div>
                                  <div class="col-6"  style='float:left'>
                                  <label class="col col-form-label">Pengaju</label>
                                       <select class='form-control' name='teid_pengaju' id='teid_pengaju' onchange="cekProject(this)" disabled="">
                                          <option></option>
                                           <?php 

                                                foreach ($list_pelaksana->result_array() as $z) {
                                                  $selected = "";
                                                  if ( $z['teid'] == $pengajunya->teid) {
                                                    $selected= "selected='selected'";
                                                  }
                                                  echo "<option value='".$z['teid']."' ".$selected.">".$z['nama_karyawan']."</option>";
                                                }
                                           ?>
                                       </select>
                                  </div>
                              ...
                              <table width='100%' id='table_project' class='table table-bordered'>
                                  <thead>
                                    <tr>
                                      ...
                                      <th width="25%">Nilai Kontrak</th>
                                      <th width="25%">Biaya Barang</th>
                                      <th width='1%'>&nbsp;</th>
                                    </tr>
                                  </thead>
                                  <tbody></tbody>
